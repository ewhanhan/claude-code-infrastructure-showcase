...
export class NotificationService {
  ...
  > = new Map();
  private CACHE_TTL = (notificationConfig.preferenceCacheTTLMinutes || 5) * 60
    * 1000;
...
import type { Prisma, User } from "@project-lifecycle-portal/database";
...
export class UserRepository {
   ...
   */
  async findActive(
    options?: { orderBy?: Prisma.UserOrderByWithRelationInput; },
  ): Promise<User[]> {
    try {
...
export class UserService {
   ...
   */
  async createUser(
    data: { email: string; name: string; roles: string[]; },
  ): Promise<User> {
