...
function analyzeFileContent(filePath: string): {
  ...
  return {
    ...
    hasPrisma:
      /prisma\.|PrismaService|findMany|findUnique|create\(|update\(|delete\(/i
        .test(
          content,
        ),
    hasController:
...
async function main() {
  try {
    ...
    if (categories.backend.length > 0) {
      const backendFiles = analysisResults.filter(f =>
        f.category === "backend"
      );
    ...
    if (categories.frontend.length > 0) {
      const frontendFiles = analysisResults.filter(f =>
        f.category === "frontend"
      );
